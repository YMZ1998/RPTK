...
from src.feature_extraction.generate_radiomics_feature_map import RadiomicsMapGenerator
...
class FeatureSelectionPipeline:
    ...
    def check_for_nan(self):
        ...
        if "ID" in self.data.columns:
            if self.data["ID"].astype(str).str.isnumeric().any():
                if "Image" in self.data.columns:
                    self.data["ID"] = self.data.index.apply(
                        lambda x: str(os.path.basename(experiment["Image"]).split('_')[0]))
        ...
        nan_columns = list(self.data.loc[:,
                           self.data.isnull().all()].columns)  # list(self.data.columns[self.data.isnull().all()].values)
    ...
    def mlxtend_feature_selector(self, model, direction: str, n_features=None, X=None, y=None):
        ...
        self.logger.info('Best ' + str(self.scoring) + ' score from ' + str(type(model).__name__) + ' for ' + str(
            direction) + ' selection: %.3f' % sfs.k_score_)
        ...
        if self.n_features is None:
            ...
            if len(X.columns) > self.critical_feature_size:
                ...
                if self.sfs_lib == "mlxtend":
                    ...
                    self.error.warning(
                        "Number of features are above critical feature count {}. Feature selection may take very long. Therefore, best features between {} and {} are selected.".format(
                            str(self.critical_feature_size), str(self.min_feature), str(self.max_feature)))
                    print(
                        "Number of features are above critical feature count {}. Feature selection may take very long. Therefore, best features between {} and {} are selected.".format(
                            str(self.critical_feature_size), str(self.min_feature), str(self.max_feature)))
                else:
                    self.error.warning(
                        "Number of features are above critical feature count {}. Feature selection may take very long. Change feature selector to mlxtend to reduce computation for feature selection.".format(
                            str(self.critical_feature_size)))
                    print(
                        "Number of features are above critical feature count {}. Feature selection may take very long. Change feature selector to mlxtend to reduce computation for feature selection.".format(
                            str(self.critical_feature_size)))
    ...
    def select_features(self):
        ...
        if os.path.isfile(str(self.model_save_dir) + "/" + "Feature_Selection_Model_Performance.csv"):
            Evaluation = pd.read_csv(str(self.model_save_dir) + "/" + "Feature_Selection_Model_Performance.csv",
                                     index_col=0)
        ...
        if not os.path.isfile(self.out_file_path):
            ...
            for model in self.model:
                ...
                got_model = os.path.isfile(
                    str(self.model_save_dir) + "/" + str(type(model).__name__) + "_trained_after_selection.sav")
                ...
                if not got_model:
                    pbar.set_description(
                        "Perform " + self.extractor + " Feature Selection with " + str(type(model).__name__))
                    ...
                    if all_comb < 1000000:
                        self.logger.info(
                            "Exhaustive feature selection is feasible for this data with {} features.".format(
                    ...
                    if self.forward_sfs:
                        ...
                        if not self.check_performed_selection(model=model,
                            ...
                            f_selected, f_best_selected = self.sequential_feature_selection(direction="forward",
                                                                                            model=model)
                    ...
                    if self.backward_sfs:
                        ...
                        if not self.check_performed_selection(model=model,
                            ...
                            b_selected, b_best_selected = self.sequential_feature_selection(direction="backward",
                                                                                            model=model)
                    ...
                    print(
                        "Performance before feature selection with {} Features.".format(str(len(self.X_train.columns))))
                    ...
                    if self.task == "binary_classification":
                        ...
                        before_evaluation = self.trainer.extended_eval(y_true=self.y_test, X=self.X_test,
                                                                       model=before_model)
                        ...
                        # Save Model:
                        if not os.path.isfile(str(self.model_save_dir) + "/" + str(
                            type(model).__name__) + "_trained_after_selection.sav"):
                            self.trainer.save_model(model=before_model,
                                                    file_path=str(self.model_save_dir) + "/" + str(
                                                        type(model).__name__) + "_trained_before_selection.sav")
                        ...
                        after_evaluation = self.trainer.extended_eval(y_true=self.y_test,
                                                                      X=self.X_test[selected_features4model],
                                                                      model=after_model)

                        after_evaluation["Origin"] = "After_Feature_Selection"
                        self.logger.info(
                            "AUROC Performance of model after feature selection: " + str(
                                after_evaluation["AUC"].values[0]))
            ...
            if os.path.isfile(str(self.model_save_dir) + "/" + "Feature_Selection_Model_Performance.csv"):
                if added_evaluation > 0:
                    Evaluation.to_csv(str(self.model_save_dir) + "/" + "Feature_Selection_Model_Performance.csv",
                                      mode='a', index=False, header=False)  # add to csv file for each model
            else:
                Evaluation.to_csv(str(self.model_save_dir) + "/" + "Feature_Selection_Model_Performance.csv",
                                  index=False)
            ...
            if ("ID" in self.data.columns) or (self.data.index.name == "ID"):
                ...
                self.error.warning(
                    "No ID column found in input data. Please add manually to output file {}".format(
                        self.sel_file_path))
            ...
            if "config" in self.data.columns:
                ...
                self.error.warning(
                    "Configuration of the feature format is wrong! Please check the format of the features before selection.")
            ...
            if not os.path.isfile(self.out_file_path):
                ...
                if selected_feature_space.columns.str.startswith('ROI_Label').any():
                    selected_feature_space = selected_feature_space.loc[:,
                                             ~selected_feature_space.columns.str.startswith('ROI_Label')]
            ...
            if self.extractor == "PyRadiomics":
                ...
                if self.modality == "CT":
                    self.extraction_yaml = " /src/config/PyRadiomics/CT.yaml"
                elif self.modality == "MRI" or self.modality == "MR":
                    self.extraction_yaml = " /src/config/PyRadiomics/MRI.yaml"
                ...
                if self.data.index.name != "config":
                    if "config" in self.data.columns:
                        ...
                        self.error.warning(
                            "No configuration column found in data. Please add configuration column to data!")
                ...
                if "Image" in selected_feature_space.columns:
                    for i, row in tqdm(selected_feature_space.iterrows(), total=len(selected_feature_space),
        ...
        for col in selected_feature_space.copy().columns:
            if col == "ID":
                if selected_feature_space.index.name == "ID":
                    ...
                    print(
                        f"Warning: {col} found in feature space! This feature does not have any relevant information! Dropping ...")
                    self.error.warning(
                        f"{col} found in feature space! This feature does not have any relevant information! Dropping ...")
                else:
            ...
            if col == "ROI_Label":
                print(
                    "Warning: ROI_Label found in feature space! This feature does not have any relevant information! Dropping ...")
                self.error.warning(
                    "ROI_Label found in feature space! This feature does not have any relevant information! Dropping ...")
        ...
        IBSIFeatureFormater(extractor=self.extractor,
                            ...
                            output_path=os.path.dirname(
                                self.out_folder) + "/selected_features/IBSI_profile/").format_features()
