...
from src.config.InputDataStatistics import DataStatGenerator
from src.config.Log_generator_config import LogGenerator
from src.config.structure_generator import out_dir_generator
from src.feature_extraction.Extractor import Extractor
...
from src.feature_selection.FeatureSelector import FeatureSelectionPipeline
from src.model_training.ModelTrainer import ModelTrainer
...
class RPTK:
    ...
    def __init__(self,
                 ...
                 input_reformat: bool = True,
                 # If your input file names are non-unique (Image or Mask path in input csv)
                 ...
                 calculate_for_every_segmentation: bool = False,
                 # calculate radiomics for every segmentation or mean all features of all segmentations from the same image
                 additional_rois_to_features: bool = False,
                 # if you have multiple ROIs per sample but want to extract radiomics features for each ROI(recommended for ROIs if they have different meaning). Additional ROIs need to have a description in Mask_Transformation column.
                 imbalance_method: str = "SMOTE",
                 # oversampling method if class imbanace has been detected in the data None is no imablance correction should be performed
                 ...
                 # variance threshold for feature filtering (min varnaice to keep feature)
                 correlation_threshold: float = None,
                 # correlation threshold for feature filtering (min correlation to identify feature clsuter)
                 ICC_threshold: float = None,
                 # ICC threshold for feature filtering (min ICC to keep feature as stable features only spplicable if segmentation perturbation is enabled)
                 ...
                 peritumoral_seg: bool = None,
                 # whether to perform peritumoral segmenttion (segmentation around the voi)
        ...
        if not self.delta:
            if self.take_label_changes:
                self.error.error(
                    "Label changes can only be taken into account if delta is enabled for calculating delta radiomics!")
                raise ValueError(
                    "Label changes can only be taken into account if delta is enabled for calculating delta radiomics!")
        ...
        if self.resample_slice_thickness_threshold is None:
            self.resample_slice_thickness_threshold = self.config["Preprocessing_config"][
                "resample_slice_thickness_threshold"]
        ...
        print("\t\t\tRPTK calculate for every segmentation of the same image: " + str(
            self.calculate_for_every_segmentation))
    ...
    def check_input_format(self, entry: pd.Series):
        ...
        # get dataframe
        df = self.data.copy().loc[
            (self.data.copy()["Mask"] == entry["Mask"]) & (self.data.copy()["Image"] == entry["Image"])]
    ...
    def plot_custom_radiomics_heatmap(self, df, prediction_labels, additional_params=None, normalize=False,
                                      normalization_method='zscore', method='average', figsize=(20, 15), y_labelpad=10,
                                      title=None, save_path=None, no_feature_names=False,
                                      prediction_label_meaning=None):
        """
        ...
        grid = fig.add_gridspec(nrows=2 + num_param_rows, ncols=4, width_ratios=width_ratios,
        ...
        ax_labels.imshow(np.array([label_color_map[lbl] for lbl in sorted_labels]).reshape(1, -1, 3), aspect='auto',
                         extent=[0, sorted_df.shape[0], 0, 1])
        ...
        if additional_params and len(additional_params) > 0:
            ...
            param_values = [np.array(additional_params[key])[sorted_idx] for key in
                            param_keys]  # Apply same sorting order
            ...
            for i, (param, values) in enumerate(zip(param_keys, param_values)):
                ...
                ax_param.imshow(values.reshape(1, -1), aspect='auto', cmap='viridis',
                                extent=[0, sorted_df.shape[0], 0, 1])
                ax_param.set_xticks([])
                ax_param.set_yticks([])
        ...
        if prediction_label_meaning is not None:
            ...
            prediction_label_handles = [Patch(facecolor=label_color_map[lbl], label=f'Label {lbl}') for lbl in
                                        unique_labels]
            ax_legend_prediction_label = fig.add_subplot(grid[1, 2])
            ax_legend_prediction_label.legend(handles=prediction_label_handles, loc='upper center',
                                              title=prediction_labels.name, fontsize=18, title_fontsize=20)
    ...
    def rename_radiomics_columns(self,
                                 ...
                                 clinical_classes: Iterable[str] = (
                                     "Clinical_features", "Clinical features", "Clinical", "Clinical feature",
                                     "Clinical_feature"),
    ...
    def generate_feature_overview_map(self,
        ...
        if os.path.isfile(
            self.out_path + extractor + "/filtered_features/" + extractor + "_General_Feature_Profile.csv"):
            # get feature profile
            feature_profile = pd.read_csv(
                self.out_path + extractor + "/filtered_features/" + extractor + "_General_Feature_Profile.csv",
                index_col=0)

            feature_profile["Feature_Class"] = feature_profile["Feature_Class"].replace(
                "neighbourhood_grey_tone_difference_matrix", "NGTDM")
            feature_profile["Feature_Class"] = feature_profile["Feature_Class"].replace(
                "grey_level_co-occurrence_matrix", "GLCM")
            feature_profile["Feature_Class"] = feature_profile["Feature_Class"].replace(
                "grey_level_distance_zone_matrix", "GLDZM")
            feature_profile["Feature_Class"] = feature_profile["Feature_Class"].replace("grey_level_run_length_matrix",
                                                                                        "GLRLM")
            feature_profile["Feature_Class"] = feature_profile["Feature_Class"].replace("grey_level_size_zone_matrix",
                                                                                        "GLSZM")
            feature_profile["Feature_Class"] = feature_profile["Feature_Class"].replace(
                "neighbouring_grey_level_dependence_matrix", "NGLDM")
            feature_profile["Feature_Class"] = feature_profile["Feature_Class"].replace("intensity-volume_histogram",
                                                                                        "IVH")
            feature_profile["Feature_Class"] = feature_profile["Feature_Class"].replace("intensity-based_statistics",
                                                                                        "IS")
            feature_profile["Feature_Class"] = feature_profile["Feature_Class"].replace("intensity_histogram", "IH")
            feature_profile["Feature_Class"] = feature_profile["Feature_Class"].replace("morphological",
                                                                                        "Morphological")
            ...
            features.columns = [col.split('_', 1)[1] if ('_' in col) and (col not in clinical_features) else col for col
                                in features.columns]
            ...
            self.plot_custom_radiomics_heatmap(features,
                                               ...
                                               title=title,
                                               save_path=out_folder + "/" + extractor + "_" + str(
                                                   self.RunID) + "_Radiomics_Feature_Clustering.png",
                                               no_feature_names=False,
                                               prediction_label_meaning=prediction_label_meaning)
        else:
            self.error.warning(
                f"Could not find the General Feature Profile for Feature class checkup! File not found: {self.out_path + extractor + '/filtered_features/' + extractor + '_General_Feature_Profile.csv'}")
            print(
                f"Warning: Could not find the General Feature Profile for Feature class checkup! File not found: {self.out_path + extractor + '/filtered_features/' + extractor + '_General_Feature_Profile.csv'}")
    ...
    def configure_clinical_data(self, clinical_df: pd.DataFrame = None):
        ...
        for feature in features_with_na:
            self.logger.info(feature + " " + str(
                np.round(np.round(clinical_df[feature].isnull().mean(), 4) * 100, 2)) + " % missing values")
            ...
            if clinical_df[feature].dtype.name == 'category':
                ...
                clinical_df[feature] = simple_imp.fit_transform(
                    pd.DataFrame(clinical_df[feature]))  # Need to convert to pd.DataFrame
    ...
    def check_input_csv_format(self):
        ...
        if self.data is not None:
            ...
            reformat = False
            self.data.drop(self.data.columns[self.data.columns.str.contains('unnamed', case=False)], axis=1,
                           inplace=True)
            ...
            with tqdm(total=len(self.data), desc='Check image input format') as pbar:
                with concurrent.futures.ProcessPoolExecutor(max_workers=self.n_cpu) as executor:
                    ...
                    for results in executor.map(self.check_input_format,
                                                [row for i, row in self.data.copy().iterrows()], chunksize=self.n_cpu):
                        pbar.update(1)
                        out.append(results)
            ...
            if self.need_to_convert:
                ...
                for msk in tqdm(self.data.copy()["Mask"].to_list(), desc="Include converted mask files"):
                    if "/converted/" not in msk:
                        ...
                        for conv_file in converted_files:
                            if os.path.basename(conv_file)[:-len(".nii.gz")] == os.path.basename(msk)[
                                                                                :-len(str(format))]:
                ...
                for img in tqdm(self.data.copy()["Image"].to_list(), desc="Include converted image files"):
                    if "/converted/" not in img:
                        ...
                        for conv_file in converted_files:
                            if os.path.basename(conv_file)[:-len("nii.gz")] == os.path.basename(img)[
                                                                               :-len(str(format))]:
            ...
            if reformat:
                ...
                self.data = self.validate_and_correct_pairs(df=self.data.copy(),
                                                            image_folder=self.out_path + "input_reformated/img/",
                                                            mask_folder=self.out_path + "input_reformated/msk/")
    ...
    def validate_sample_masks(self, df):
        ...
        def process_sample_group(sample_group):
            ...
            try:
                ...
                invalid_peritumoral_masks = peritumoral_masks[~peritumoral_masks['Mask'].apply(
                    lambda filename: bool(
                        re.search(r'roiT_[0-9]*_roiN_[0-9]*(_[0-9]*[_]*peritumoral)', str(os.path.basename(filename))))
                )]['Mask']
            except Exception as ex:
                self.error.error(
                    f"It seems like peritumoral segmentation for sample {sample} is not correctly formated or missing! Check the preprocessing files for peritumoral segmentations. " + str(
                        ex))
                raise ValueError(
                    f"It seems like peritumoral segmentation for sample {sample} is not correctly formated or missing! Check the preprocessing files for peritumoral segmentations. " + str(
                        ex))
            ...
            if len(invalid_peritumoral_masks) > 0:
                print(f"Found {len(invalid_peritumoral_masks)} invalid Peritumoral masks for sample {sample}: ",
                      invalid_peritumoral_masks.to_list())
                self.error.warning(
                    f"Found {len(invalid_peritumoral_masks)} invalid Peritumoral masks for sample {sample}: {invalid_peritumoral_masks.to_list()}")
        ...
        if "Mask_Transformation" in df.columns:
            if "Peritumoral" in df["Mask_Transformation"].values.tolist():
                ...
                cleaned_df = cleaned_df.groupby('ID', group_keys=False).apply(process_sample_group).reset_index(
                    drop=False)
            else:
                cleaned_df = df.copy()  # No Peritumoral mask transformation found in data
                self.error.warning(
                    "Could not find any peritumoral mask transformation in data! Check the data if this is correct.")
        else:
    ...
    def get_preprocessor(self, perturbation_method: list = None):
        ...
        SegPro = SegProcessor(rptk_config_json=self.rptk_config_json,
                              ...
                              resample_slice_thickness_threshold=int(
                                  self.config["Preprocessing_config"]["resample_slice_thickness_threshold"]),
    ...
    def filter_features(self, extracted_features=None, return_=False, stability_filtering=None, format_check=True,
                        peritumoral=True, out_path=None):
        """
            ...
            if "ID" not in self.filtered_features[extractor].columns:
                if "ID" != self.filtered_features[extractor].index.name:
                    if os.path.isfile(out_path + extractor + "/filtered_features/filtered_features.csv"):
                        self.filtered_features[extractor] = pd.read_csv(
                            out_path + extractor + "/filtered_features/filtered_features.csv", index_col=0)
                    else:
                        print("Warning: ID is missing in filtered features and output file {} is missing!".format(
                            out_path + extractor + "/filtered_features/filtered_features.csv"))
                        self.error.warning("ID is missing in filtered features and output file {} is missing!".format(
                            out_path + extractor + "/filtered_features/filtered_features.csv"))
        ...
        if ("ID" not in self.filtered_features[extractor].columns) & (
            "ID" != self.filtered_features[extractor].index.name):
    ...
    def select_features(self, filtered_features=None, return_=False, n_features=None, model=None, backward_sfs=True,
                        forward_sfs=True, critical_feature_size=None, min_feature=None, max_feature=None,
                        rand_state=None, out_folder=None):
        """
        ...
        if self.feature_selection:
            ...
            for extractor in filtered_features:
                ...
                if len(self.clinical_df) > 0:
                    ...
                    if "config" not in features.columns:
                        ...
                        if features.index.name == "config":
                            ...
                            features['config'] = features['Image'].apply(self.process_filename) + "_" + features[
                                'Mask'].apply(self.process_filename)
                    ...
                    for id_ in self.clinical_df["ID"]:
                        for conf in features["config"]:
                            if str(conf).startswith(str(id_)):
                                features.loc[features["config"] == conf, "ID"] = \
                                    self.clinical_df.loc[self.clinical_df["ID"] == id_, "ID"].values[0]
                        ...
                        if features.index.duplicated().any:
                            ...
                            if len(self.clinical_df) < len(features):
                                ...
                                for id in features.index.values.tolist():
                                    ...
                                    tmp = pd.concat([features[features.index == id], self.clinical_df[
                                        self.clinical_df["ID"] == features.loc[features.index == id, "ID"].values[0]]],
                                                    keys=["ID"], axis=0, ignore_index=True)
                ...
                features, non_normalied_features = self.normalize_if_needed(features.copy(),
                                                                            self.clinical_df.columns.to_list())
                ...
                if isinstance(rand_state, list):
                    for seed in rand_state:
                        ...
                        if extractor in self.selected_features:
                            self.selected_features[extractor] = pd.concat(
                                [self.selected_features[extractor], self.feature_selector.select_features()], axis=1)
                        else:
            ...
            self.logger.info(
                "Feature selection disabled! Set 'feature_selection' to True in the configuration file to enable it.")
    ...
    def summarize_selected_features_from_filtering(self,
        ...
        if len(available_selected_features) == 0:
            print("Could not find selected features with kernels" + str(
                most_common_kernels) + " check configuration between filtered featureas and selected features")
            self.error.error("Could not find selected features with kernels" + str(
                most_common_kernels) + " check configuration between filtered featureas and selected features")
            raise ValueError("Could not find selected features with kernels" + str(
                most_common_kernels) + " check configuration between filtered featureas and selected features")
    ...
    def get_features_without_kernel(self, available_selected_features, filtered_features):
        ...
        for kernel in available_selected_features:
            ...
            transformed_features_tmp = transformed_features_tmp.loc[:,
                                       ~transformed_features_tmp.columns.duplicated()].copy()
    ...
    def generate_augmented_feature_space(self, extractor=None, selected_features=None, filtered_features=None,
                                         n_best_kernels=None, mask_transformations=None):
        """
        ...
        selected_feature_profile = FeatureFormatter(features=selected_features[extractor].columns,
                                                    ...
                                                    logger=self.logger,
                                                    error=self.error,
                                                    output_path=self.out_path + extractor + "/selected_features/").exe(
            title="Selected_Feature_Profile")
        ...
        kernels_in_selected_features, selected_features_wo_kernel, selected_peritumoral_features = self.extract_selected_kernels(
            extractor,
            selected_features,
        ...
        available_selected_features, original_selected_features = self.summarize_selected_features_from_filtering(
            filtered_features,
            selected_peritumoral_features,
            original_selected_features,
            selected_features_wo_kernel,
        ...
        if not mask_transformations is None:
            mask_transformed_features = self.get_features_without_kernel(available_selected_features,
    ...
    def predict(self,
        ...
        for extractor in selected_features:
            ...
            if include_augmentations:
                ...
                if len(glob.glob(
                    self.out_path + extractor + "/filtered_features/tmp/*_normalized_data_wo_mask_transformation.csv")) > 0:
                    # Get filtered features from 
                    filtered_features_path = glob.glob(
                        self.out_path + extractor + "/filtered_features/tmp/*_normalized_data_wo_mask_transformation.csv")[
                        0]
                else:
                    self.error.error(
                        "Could not find filtered features to condense the feature space. Please check if a file with ending _normalized_data_wo_mask_transformation.csv is in {} !".format(
                            str(self.out_path + extractor + "/filtered_features/tmp/")))
                    raise ValueError(
                        "Could not find filtered features to condense the feature space. Please check if a file with ending _normalized_data_wo_mask_transformation.csv is in {} !".format(
                            str(self.out_path + extractor + "/filtered_features/tmp/")))
                ...
                if not mask_transformations is None:
                    if not isinstance(mask_transformations, pd.DataFrame):
                        if not isinstance(mask_transformations, str):
                            mask_filtered_features_path = glob.glob(
                                self.out_path + extractor + "/filtered_features/tmp/*_normalized_data_mask_transformation.csv")[
                                0]
            ...
            selected_features_extractor = selected_features_extractor.loc[:,
                                          ~selected_features_extractor.columns.str.startswith('config_')]
            selected_features_extractor = selected_features_extractor.loc[:,
                                          ~selected_features_extractor.columns.str.startswith('Raw_')]
            ...
            if reuse_split:
                ...
                train_id_path = self.out_path + extractor + "/selected_features/Train_idx_" + str(
                    self.RunID) + "_" + str(self.rand_state) + ".csv"
                test_id_path = self.out_path + extractor + "/selected_features/Test_idx_" + str(self.RunID) + "_" + str(
                    self.rand_state) + ".csv"
                val_id_path = self.out_path + extractor + "/selected_features/Val_idx_" + str(self.RunID) + "_" + str(
                    self.rand_state) + ".csv"
            ...
            if repeat_prediction:
                for rand in seed:
                    ...
                    self.model_trainer = ModelTrainer(data=selected_features_extractor.copy(),
                                                      ...
                                                      train_idx=self.train_ids,
                                                      # ID of training samples is specific samples are needed
                                                      test_idx=self.test_ids,
                                                      # ID of test samples is specific samples are needed
                                                      val_idx=self.val_ids,
                                                      # ID of validate samples is specific samples are needed
